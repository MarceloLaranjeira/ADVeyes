...
-- O reconciliador antigo pausava falhas transitórias ao esgotar a escala
-- curta. Reativa somente esse estado legado; erros permanentes preservam a
-- pausa e continuam exigindo intervenção humana.
update public.legal_sync_sources
set active = true,
    paused_reason = null,
    failure_count = least(failure_count, 4),
    next_sync_at = now(),
    updated_at = now()
where paused_reason = 'max_retries'
...
-- Falha ativa e fonte interrompida são estados distintos; os contadores não
-- devem somar a mesma fonte nas duas categorias.
...
select
  ...
  count(*) filter (
    where active and last_error_code = 'integration_not_configured'
