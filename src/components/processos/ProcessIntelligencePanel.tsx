...
export function ProcessIntelligencePanel({ tenantId, processId }: { tenantId: string; processId: string }) {
  ...
  return (
    <Card className="mb-5 overflow-hidden border-primary/20"><CardContent className="p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Bot className="h-4 w-4 text-primary" /><h2 className="font-serif text-lg font-semibold">Inteligência processual</h2>{insight ? <Badge variant="outline">{RISK_LABELS[insight.risk]}</Badge> : null}</div><p className="mt-1 text-xs text-muted-foreground">Leitura operacional baseada nos andamentos e prazos vinculados.</p></div><Button variant="outline" size="sm" className="gap-2" disabled={query.analyze.isPending} onClick={() => void analyze()}>{query.analyze.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}{insight ? "Reanalisar" : "Analisar agora"}</Button></div>
