          ...
          <CardContent className="p-6">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <div className="relative flex-1 min-w-[250px]">
                ...
                <Input placeholder="Número do processo (ex: 0001234-56.2024.8.04.0001)" className="pl-10 h-11" value={numero} onChange={(e) => setNumero(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscar()} />
              </div>
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <Button variant="outline" size="sm" className="gap-1 text-xs" onClick={() => monitorar(p.numero)} disabled={monitorando.includes(p.numero)}>
                      <Bell className="w-3 h-3" /> {monitorando.includes(p.numero) ? "Monitorando" : "Monitorar"}
                    </Button>
                    <Button variant="outline" size="sm" className="gap-1 text-xs" onClick={() => peticionar(p.numero)}>
                ...
                {p.dataAjuizamento && (
                  <div className="flex items-center gap-1 text-xs text-muted-foreground mb-3">
                    <Calendar className="w-3 h-3" /> Ajuizado em: {new Date(p.dataAjuizamento).toLocaleDateString("pt-BR")}
                  </div>
                )}
