...
export default function Onboarding() {
  ...
  if (!currentTenant || !onboarding) return <div className="grid min-h-screen place-items-center bg-background"><div className="h-9 w-9 animate-spin rounded-full border-b-2 border-primary" /></div>;
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-3 sm:p-6 lg:p-10">
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-6xl overflow-hidden rounded-[28px] bg-white shadow-2xl shadow-slate-900/10 lg:grid-cols-[0.86fr_1.14fr]">
        <aside className="relative overflow-hidden bg-sidebar p-8 text-white sm:p-12">
        ...
        <section className="flex items-center justify-center p-7 sm:p-12 lg:p-16"><div className="w-full max-w-xl">
          <div className="mb-10 flex gap-2">{[0, 1, 2].map((step) => <span key={step} className={`h-1.5 flex-1 rounded-full ${step <= (teamStep ? 2 : 1) ? "bg-primary" : "bg-slate-200"}`} />)}</div>
          {!teamStep ? <>
            <p className="text-sm font-semibold text-primary">Etapa 2 de 3</p><h2 className="mt-2 text-3xl font-semibold text-foreground">Cadastre sua OAB</h2><p className="mt-3 leading-relaxed text-slate-600">Usaremos esses dados para procurar processos nas bases públicas e conciliar publicações do Diário da Justiça.</p>
            ...
            <Button className="mt-7 h-12 w-full" disabled={working || !professionalId || !oabNumber} onClick={() => void saveOab()}>{working ? "Salvando OAB e ativando monitoramento..." : <>Buscar processos e ativar monitoramento <ArrowRight className="ml-2 h-4 w-4" /></>}</Button>
            ...
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-green-100 text-green-700"><Check className="h-8 w-8" /></div><p className="mt-6 text-sm font-semibold text-primary">Etapa 3 de 3</p><h2 className="mt-2 text-3xl font-semibold text-foreground">Tudo certo para começar</h2>
            ...
            <div className="mt-8 rounded-2xl border bg-background p-5 text-left"><div className="flex gap-3"><Users className="mt-0.5 h-5 w-5 text-primary" /><div><p className="font-semibold text-foreground">Trabalha com alguém?</p><p className="mt-1 text-sm text-slate-500">Você pode convidar um colaborador agora ou fazer isso depois.</p></div></div></div>
            <Button className="mt-6 h-12 w-full" disabled={working} onClick={() => void finish(false)}>{working ? "Concluindo..." : "Entrar no meu painel"}</Button><Button className="mt-3 w-full" variant="outline" disabled={working} onClick={() => void finish(true)}>Convidar colaborador</Button>
          </div>}
        </div></section>
      </div>
    </main>
