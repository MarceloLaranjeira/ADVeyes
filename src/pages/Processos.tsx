...
const OfficialProcessSearch = lazy(() => import("@/pages/BuscaJurisprudencia"));
...
export default function Processos() {
  ...
  return (
    <AppLayout><div className="animate-fade-in space-y-5">
      ...
      <Tabs value={tab} onValueChange={value => updateRoute({ tab: value as ProcessTab })}>
        <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto p-1 sm:w-auto">
          ...
          <TabsTrigger value="consulta">Consulta oficial</TabsTrigger>
        </TabsList>
      </Tabs>

      {tab === "consulta" ? (
        <Suspense fallback={<div className="flex min-h-72 items-center justify-center rounded-xl border"><Loader2 className="h-7 w-7 animate-spin text-primary" /><span className="ml-3 text-sm text-muted-foreground">Carregando a consulta oficial...</span></div>}>
          <OfficialProcessSearch embedded initialQuery={filters.search} initialTribunal={searchParams.get("tribunal") ?? undefined} />
        </Suspense>
      ) : <>
        ...
        </div><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap gap-2"><Button variant={filters.stalledOnly ? "secondary" : "outline"} size="sm" onClick={() => setFilters({ stalledOnly: !filters.stalledOnly, pendingOnly: false })}>Somente sem avanço</Button>{filters.pendingOnly ? <Button variant="secondary" size="sm" onClick={() => setFilters({ pendingOnly: false })}>Aguardando análise</Button> : null}</div><div className="flex items-center gap-2"><span className="text-xs text-muted-foreground">{filtered.length} de {inSituation.length} processos</span><Button variant="ghost" size="sm" className="gap-1" onClick={() => updateRoute({ filters: EMPTY_INTELLIGENCE_FILTERS, limit: PROCESS_PAGE_SIZE })}><FilterX className="h-3.5 w-3.5" /> Limpar</Button><Button variant="ghost" size="icon" aria-label="Atualizar central" onClick={() => void intelligence.refetch()}><RefreshCw className={`h-4 w-4 ${intelligence.loading ? "animate-spin" : ""}`} /></Button></div></div></div>

        {intelligence.loading ? <div className="flex min-h-72 items-center justify-center rounded-xl border"><Loader2 className="h-7 w-7 animate-spin text-primary" /><span className="ml-3 text-sm text-muted-foreground">Lendo a carteira processual...</span></div> : intelligence.error ? <div role="alert" className="rounded-xl border border-destructive/30 p-8 text-center"><p className="font-semibold">Não foi possível carregar a inteligência processual</p><Button className="mt-4" onClick={() => void intelligence.refetch()}>Tentar novamente</Button></div> : <><div className="mt-4">{tab === "central" ? <IntelligenceCentral items={visibleItems} onCorrect={setSelected} /> : tab === "pipeline" ? <IntelligencePipeline items={visibleItems} /> : <IntelligenceList items={visibleItems} />}</div>{visibleItems.length < filtered.length ? <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed p-4"><p className="text-xs text-muted-foreground">Exibindo {visibleItems.length} de {filtered.length} processos para manter a Central rápida.</p><Button variant="outline" onClick={() => updateRoute({ limit: limit + PROCESS_PAGE_SIZE })}>Mostrar mais 40</Button></div> : null}</>}
      </>}
